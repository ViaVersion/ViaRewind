			...
			public void registerMap() {
				...
				handler(wrapper -> {
					...
					if (gameProfile != null && gameProfile.gamemode == 3) {
						...
						for (short i = 0; i < 4; i++) {
							equipmentPacket = new PacketWrapper(0x04, null, wrapper.user());
							equipmentPacket.write(Type.INT, entityId);
					...
					tracker.addPlayer(wrapper.get(Type.VAR_INT, 0), uuid);
				});
				...
				handler(wrapper -> {
					List<Metadata> metadata = wrapper.get(Types1_7_6_10.METADATA_LIST, 0);  //Metadata
			...
			public void registerMap() {
				...
				handler(wrapper -> {
					...
					if (typeId == 71) {
						...
						wrapper.cancel();
						EntityTracker tracker = wrapper.user().get(EntityTracker.class);
						ArmorStandReplacement armorStand = new ArmorStandReplacement(entityId, wrapper.user());
					...
					wrapper.set(Type.BYTE, 0, typeId);
					...
					if (type != null && type.isOrHasParent(Entity1_10Types.EntityType.FALLING_BLOCK)) {
						...
						wrapper.set(Type.INT, 3, data = (state.getId() | state.getData() << 16));
					}
			...
			public void registerMap() {
				...
				handler(wrapper -> {
					...
					if (typeId == 30) {
						...
						EntityTracker tracker = wrapper.user().get(EntityTracker.class);
						GuardianReplacement guardian = new GuardianReplacement(entityId, wrapper.user());
						...
						EntityTracker tracker = wrapper.user().get(EntityTracker.class);
						EndermiteReplacement endermite = new EndermiteReplacement(entityId, wrapper.user());
