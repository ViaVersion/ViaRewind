...
public class MetadataRewriter1_8To1_9 extends VREntityRewriter<ClientboundPackets1_9, Protocol1_8To1_9> {
	...
	private void handleMetadata(MetaHandlerEvent event, Metadata metadata) {
		...
		if (metadata.id() == MetaIndex.ENTITY_STATUS.getIndex()) {
			tracker.getStatus().put(event.entityId(), ((Byte) metadata.getValue()).intValue());
		}
			...
			if (metaIndex != null) {
				if (metaIndex.getOldType() == null || metaIndex.getNewType() == null) {
					...
					if (metaIndex == MetaIndex.PLAYER_HAND) {
						byte status = (byte) tracker.getStatus().getOrDefault(event.entityId(), (byte) 0);
				...
				switch (metaIndex.getNewType()) {
					...
					case OptUUID:
						...
						if (owner == null) metadata.setValue("");
						else metadata.setValue(owner.toString());
						break;
					case BlockID:
						event.cancel();
					...
					case String:
						...
						break;
					case Boolean:
						if (metaIndex == MetaIndex.AGEABLE_CREATURE_AGE) metadata.setValue((byte) ((Boolean) value ? -1 : 0));
						else metadata.setValue((byte) ((Boolean) value ? 1 : 0));
						break;
					case Slot:
