...
public class ScoreboardPacketRewriter1_8 {
	...
	public static void register(Protocol1_8To1_7_6_10 protocol) {
		protocol.registerClientbound(ClientboundPackets1_8.SET_OBJECTIVE, new PacketHandlers() {
			@Override
		...
		protocol.registerClientbound(ClientboundPackets1_8.SET_PLAYER_TEAM, new PacketHandlers() {
			...
			public void register() {
				map(Types.STRING);
				handler(wrapper -> {
					String team = wrapper.get(Types.STRING, 0);
					if (team == null) {
						wrapper.cancel();
						return;
					}
					...
					if (mode != 0 && !scoreboard.teamExists(team)) {
						...
						remove.write(Types.STRING, team);
						remove.write(Types.BYTE, (byte) 1);
