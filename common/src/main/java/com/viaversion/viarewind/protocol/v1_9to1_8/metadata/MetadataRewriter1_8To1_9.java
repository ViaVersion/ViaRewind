...
public class MetadataRewriter1_8To1_9 extends VREntityRewriter<ClientboundPackets1_9, Protocol1_9To1_8> {

	private static final byte HAND_ACTIVE_BIT = 0;
	private static final byte STATUS_USE_BIT = 4;

	public MetadataRewriter1_8To1_9(Protocol1_9To1_8 protocol) {
		super(protocol);
	}
	...
	protected void registerPackets() {
		...
		protocol.registerClientbound(ClientboundPackets1_9.SPAWN_ENTITY, new PacketHandlers() {
			...
			public void register() {
				...
				map(Type.DOUBLE, Protocol1_9To1_8.DOUBLE_TO_INT_TIMES_32); // X
				map(Type.DOUBLE, Protocol1_9To1_8.DOUBLE_TO_INT_TIMES_32); // Y
				map(Type.DOUBLE, Protocol1_9To1_8.DOUBLE_TO_INT_TIMES_32); // Z
				map(Type.BYTE); // Pitch
				map(Type.BYTE); // Yaw
				map(Type.INT); // Data
		...
		protocol.registerClientbound(ClientboundPackets1_9.SPAWN_EXPERIENCE_ORB, new PacketHandlers() {
			...
			public void register() {
				...
				handler(wrapper -> {
					final int entityId = wrapper.get(Type.VAR_INT, 0);
					wrapper.user().getEntityTracker(Protocol1_9To1_8.class).addEntity(entityId, EntityTypes1_10.EntityType.EXPERIENCE_ORB);
				});
			}
		});
		...
		protocol.registerClientbound(ClientboundPackets1_9.SPAWN_GLOBAL_ENTITY, new PacketHandlers() {
			...
			public void register() {
				...
				handler(wrapper -> {
					final int entityId = wrapper.get(Type.VAR_INT, 0);
					wrapper.user().getEntityTracker(Protocol1_9To1_8.class).addEntity(entityId, EntityTypes1_10.EntityType.LIGHTNING);
				});
			}
		});
		...
		protocol.registerClientbound(ClientboundPackets1_9.SPAWN_PAINTING, new PacketHandlers() {
			...
			public void register() {
				...
				handler(wrapper -> {
					final int entityId = wrapper.get(Type.VAR_INT, 0);
					wrapper.user().getEntityTracker(Protocol1_9To1_8.class).addEntity(entityId, EntityTypes1_10.EntityType.PAINTING);
				});
			}
		});
